...
def load_data(args, n_center=7):
    ...
    test_dataloader = []
    for c in range(n_center):
        train_dataloader.append(DataLoader(train_ds[c], batch_size=args.batch_size, shuffle=True, num_workers=args.workers))
...
if __name__ == "__main__":
    ...
    log = {'train_loss':[[] for i in range(n_center+1)], 'train_acc':[[] for i in range(n_center+1)], 'test_loss':[[] for i in range(n_center+1)], 'test_acc':[[] for i in range(n_center+1)], 'test_auc':[[] for i in range(n_center+1)]}    
