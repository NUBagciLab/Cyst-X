...
def load_data(args, n_center=7):
    ...
    for c in range(n_center):
        ...
        train_image1, train_image2, train_label, test_image1, test_image2, test_label = get_fold(image1_list, image2_list, label_list, fold = args.fold)
        print(f"Center {c+1} has {len(train_image1)}*2 training images and {len(test_image1)}*2 testing images")      
        train_ds.append(Dataset(data=[{"t1": img1, "t2": img2, "label": lbl} for img1, img2, lbl in zip(train_image1, train_image2, train_label)], transform=train_transforms))
        test_ds.append(Dataset(data=[{"t1": img1, "t2": img2, "label": lbl} for img1, img2, lbl in zip(test_image1, test_image2, test_label)], transform=test_transforms))
