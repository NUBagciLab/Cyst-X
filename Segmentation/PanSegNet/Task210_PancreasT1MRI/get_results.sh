...
EXTERNAL_DIR="${SCRIPT_DIR}/nnTransUNetTrainerV2__nnUNetPlansv2.1_external_T1"
...
then aggregate class_1 metrics by center into t1_result.csv.

Options:
  --internal    5-fold internal split: nnTransUNetTrainerV2__nnUNetPlansv2.1
  --external    leave-one-center-out: nnTransUNetTrainerV2__nnUNetPlansv2.1_external_T1
...
run_one() {
  ...
  local out_csv="${exp_dir}/t1_result.csv"
  echo "===== ${tag} ====="
