...
if __name__ == "__main__":
    ...
    scheduler_local = [torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_local[i], T_max=args.epochs) for i in range(n_center)]
    log = {'train_loss':[[] for i in range(n_center+1)], 'test_dice':[[] for i in range(n_center+1)]}
    test_dice_max = 0.0
    for epoch in range(1, args.epochs+1):
        for i in range(n_center):
            log['train_loss'][i].append(train_fn(model_local[i], optimizer_local[i], criterion, train_dataloader[i], device))
            scheduler_local[i].step()
        log['train_loss'][-1].append(sum([log['train_loss'][i][-1]*len(train_dataloader[i]) for i in range(n_center)])/n_train_dataloader)
        print(f"Epoch {epoch} global train loss {log['train_loss'][-1][-1]:.4f}")
        ...
        if epoch % args.val_freq == 0:
            ...
            print(f"Epoch {epoch} test dice {log['test_dice'][-1][-1]:.4f}")
